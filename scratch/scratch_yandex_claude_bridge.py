...
YANDEX_USER_DATA = r"C:\Users\Sasha  Baron\AppData\Local\Yandex\YandexBrowser\User Data"
TEMP_PROFILE = r"C:\ANTIGRAVITY\1\scratch\temp_yandex_profile"
YANDEX_PATH = r"C:\Users\Sasha  Baron\AppData\Local\Yandex\YandexBrowser\Application\browser.exe"

def prepare_temp_profile():
    print("🧹 Подготовка временного профиля...")
    ...
    # Копируем куки
    src_cookies = os.path.join(YANDEX_USER_DATA, "Default", "Network", "Cookies")
    ...
    if os.path.exists(src_cookies):
        try:
            ...
            print("✅ Файл Cookies скопирован.")
        except Exception as e:
            print(f"Ошибка копирования Cookies: {e}")
            
    # Копируем Local Storage
    src_ls = os.path.join(YANDEX_USER_DATA, "Default", "Local Storage")
    ...
    if os.path.exists(src_ls):
        try:
            ...
            print("✅ Папка Local Storage скопирована.")
        except Exception as e:
            print(f"Ошибка копирования Local Storage: {e}")

async def main():
    prepare_temp_profile()
    
    print("🚀 Запуск Playwright с временным профилем Яндекс.Браузера...")
    async with async_playwright() as p:
        ...
        screenshot_path = r"C:\ANTIGRAVITY\1\scratch\claude_temp_debug.png"
        await page.screenshot(path=screenshot_path)
        print(f"📸 Скриншот сохранен в {screenshot_path}")
        
        content = await page.content()
        if "Sign in" in content or "Continue with Google" in content:
            print("❌ Авторизация не удалась. Claude требует войти в аккаунт.")
        else:
